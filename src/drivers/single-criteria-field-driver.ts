...
export class SingleCriteriaFieldDriver implements FieldDriver {
    displayEditor = (activity: Activity, property: ActivityPropertyDescriptor): RenderResult => {
      ...
      return `<wf-single-criteria-field name="${name}" label="${label}" hint="${property.hint}" value="${expressionValue}" syntax="${value.syntax}"></wf-single-criteria-field>`;
    };
